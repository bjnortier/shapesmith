define([
    ...
    var DiskDB = function(config) {
      ...
      if (!config.root) {
        throw new Error('no root directory for disk db');
      }
      var root = path.normalize(config.root);
      // Use sync function here as this is on app startup and we want to fail
      // immediately
      if (!fs.existsSync(root)) {
        throw new Error(util.format('directory "%s" doesn\'t exist', root));
      }
      if (!fs.statSync(root).isDirectory()) {
        throw new Error(util.format('directory "%s" is not a directory', root));   
      }
      ...
      this.updateRefs = function(user, design, type, ref, refData, callback) {
        var refsPath = path.join(root, user, design, '_refs');
        fs.readFile(refsPath, function (err, currentRefs) {
          if (err) {
            callback(err);
          } else {
